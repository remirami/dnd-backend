...
def test_combat_flow():
    ...
    if not characters:
        print("  [FAIL] No characters found! Please create a character first.")
        print("   You can use Django admin or the API to create one.")
        return
    
    character = characters[0]
    print(f"  [OK] Using character: {character['name']} (ID: {character['id']})")
    
    # Get encounters
    encounters_response = SESSION.get(f"{BASE_URL}/encounters/")
    encounters = encounters_response.json().get('results', [])
    if not encounters:
        print("  [FAIL] No encounters found! Please create an encounter first.")
        return
    ...
    for enc in encounters:
        ...
        enc_detail = SESSION.get(f"{BASE_URL}/encounters/{enc['id']}/").json()
    ...
    if not encounter:
        print("  [FAIL] No encounters with enemies found! Please add enemies to an encounter first.")
        return
    
    print(f"  [OK] Using encounter: {encounter['name']} (ID: {encounter['id']})")
    
    # Use enemies from the encounter object
    encounter_enemies = encounter.get('enemies', [])
    if not encounter_enemies:
        print("  [FAIL] No enemies in encounter! Please add enemies to the encounter first.")
        return
    
    encounter_enemy = encounter_enemies[0]
    print(f"  [OK] Using enemy: {encounter_enemy['name']} (ID: {encounter_enemy['id']}) from encounter {encounter['id']}")
    ...
    initiative_response = SESSION.post(
    ...
    state_response = SESSION.get(f"{BASE_URL}/combat/sessions/{session_id}/")
    ...
    if current_participant:
        print(f"\n  [OK] Current turn: {current_participant['name']} (ID: {current_participant['id']})")
    ...
    if char_participant_id:
        ...
        try:
            ...
            condition_response = SESSION.post(
    ...
    if char_participant_id:
        ...
        if char_participant:
            ...
            concentration_spell_response = SESSION.post(
                f"{BASE_URL}/combat/sessions/{session_id}/cast_spell/",
                json={
            ...
            if spell_result and spell_result.get('action'):
                ...
                participant_detail = SESSION.get(f"{BASE_URL}/combat/participants/{char_participant['id']}/").json()
                if participant_detail.get('is_concentrating'):
                    print(f"  [OK] Concentration started on: {participant_detail.get('concentration_spell', 'Unknown')}")
    ...
    if char_participant_id:
        ...
        participant_detail = SESSION.get(f"{BASE_URL}/combat/participants/{char_participant_id}/").json()
        ...
        if participant_detail.get('is_concentrating'):
            ...
            if damage_result and damage_result.get('concentration_broken'):
                print("  [OK] Concentration check triggered and concentration was broken")
            elif damage_result:
                print("  [OK] Concentration check triggered and concentration maintained")
        else:
    ...
    if char_participant and enemy_participant:
        ...
        opp_attack_response = SESSION.post(
        ...
        if opp_result:
            print("  [OK] Opportunity attack executed")
            # Check if reaction was used
            participant_detail = SESSION.get(f"{BASE_URL}/combat/participants/{char_participant['id']}/").json()
            if participant_detail.get('reaction_used'):
                print("  [OK] Reaction marked as used")
    ...
    if char_participant_id:
        ...
        if char_participant:
            ...
            # Make a death save
            death_save_response = SESSION.post(
            ...
            if death_save_result:
                print(f"  [OK] Death save made: {death_save_result.get('message', '')}")
                print(f"  [OK] Successes: {death_save_result.get('death_save_successes', 0)}")
                print(f"  [OK] Failures: {death_save_result.get('death_save_failures', 0)}")
    ...
    if enemy_participant:
        ...
        legendary_response = SESSION.post(
        ...
        if legendary_result:
            print("  [OK] Legendary action executed")
            print(f"  [OK] Remaining legendary actions: {legendary_result.get('legendary_actions_remaining', 0)}")
        else:
    ...
    if char_participant_id:
        ...
        if char_participant:
            ...
            start_conc_response = SESSION.post(
            ...
            if start_conc_result:
                print("  [OK] Concentration started")
                
                # End concentration
                end_conc_response = SESSION.post(
    ...
    print("="*60)
    print("Phase 1:")
    print("[OK] Combat session created")
    print("[OK] Participants added")
    print("[OK] Initiative rolled")
    print("[OK] Combat started")
    print("[OK] Attack made")
    print("[OK] Turn advanced")
    print("\nPhase 2:")
    print("[OK] Spell casting tested")
    print("[OK] Saving throws tested")
    print("[OK] Conditions system tested")
    print("\nPhase 3:")
    print("[OK] Concentration spells tested")
    print("[OK] Concentration checks tested")
    print("[OK] Opportunity attacks tested")
    print("[OK] Death saving throws tested")
    print("[OK] Legendary actions tested")
    print("[OK] Concentration management tested")
    print("\n[DONE] Phase 1, 2 & 3 combat system is working!")
    print("\nTo continue testing:")
